...
CROP_COLORS = {
    "carrot": [255, 140, 0, 190],
    "potato": [139, 69, 19, 190],
    "wheat": [34, 139, 34, 190],
    "unknown": [160, 160, 160, 170],
    "other": [190, 190, 190, 150],
}
...
def crop_color(crop: str) -> list[int]:
    crop = str(crop).lower()
    if crop == "carrot":
        return [255, 140, 0, 190]
    if crop == "potato":
        return [139, 69, 19, 190]
    if crop == "wheat":
        return [34, 139, 34, 190]
...
health_filter = st.sidebar.selectbox(
    "Health",
    ["All"] + safe_values(field_master, "health_label")
...
alert_filter = st.sidebar.selectbox(
    "Alert",
    ["All"] + safe_values(field_master, "alert_level")
    if "alert_level" in field_master.columns
    else ["All"],
    index=0,
)
...
season_filter = st.sidebar.selectbox(
    "Season",
    ["All"] + safe_values(field_state, "season_year_inferred")
    if "season_year_inferred" in field_state.columns
    else ["All"],
    index=0,
)
...
stage_filter = st.sidebar.selectbox(
    "Stage",
    ["All"] + safe_values(field_state, "stage_name")
    if "stage_name" in field_state.columns
    else ["All"],
    index=0,
)
...
with tabs[0]:
    ...
    queue = (
        master_f.sort_values("inspection_rank")
        if "inspection_rank" in master_f.columns
        else master_f.copy()
    )
    queue_cols = [
        c
